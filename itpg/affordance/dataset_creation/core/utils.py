...
def split_by_ep(root_dir, min_labels, remove_blank_mask_instances, data_discovery_episodes=[], only_language=False):
    ...
    for ep in tqdm.tqdm(range(n_episodes)):
        ...
        ep_data = {
            "gripper_cam": [f.split(os.sep)[-1] for f in selected_gripper_files],
    ...
    for ep in tqdm.tqdm(range(n_episodes)):
        ...
        for split in ["validation", "training"]:
            ...
            if split == "validation":
                ...
                static_cam_files = static_data[: -len(static_data) // 4]
            ep_data = {
                "gripper_cam": [f.split(os.sep)[-1] for f in gripper_cam_files],
...
def save_dict_data(data_dict, directory, sub_dir, save_viz=False):
    ...
    if save_viz:
        data_dir, viz_frames, viz_aff = create_dirs(directory, sub_dir, ["data", "viz_frames", "viz_affordance"])
        imgKey_to_dir = {
            "frame": viz_frames,
            "viz_out": viz_aff,
        }
