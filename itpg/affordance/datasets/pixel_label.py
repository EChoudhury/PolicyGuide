...
class PixeLabelDataLang(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert (
            center < self.resize
        ).all(), "center is out of range, old_shape %s, resize %s, old_center %s, new_center %s" % (
            str(old_shape),
            str(self.resize),
            str(data["centers"][0, 1:]),
            str(center),
        )
...
def get_imgs(cfg):
    ...
    for b_idx, b in enumerate(loader):
        ...
        out_img = add_img_text(out_img, inp["lang_goal"][0], font_scale=0.6, thickness=1)
        out_img = out_img[:, :, ::-1]
        if cfg.save_viz:
            file_dir = "./imgs"
