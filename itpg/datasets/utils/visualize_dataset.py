...
def visualize_dataset(cfg: DictConfig) -> None:
    ...
    # prop_space = OmegaConf.load('conf/datamodule/observation_space/lang_rgb_static_abs_act_itpg.yaml')

    # prop_space = OmegaConf.create({"n_state_obs": 8, 
    #                             "keep_indices": [[0, 7], [14,15]], 
    #                             "robot_orientation_idx": [3, 6], 
    #                             "normalize": True, 
    #                             "normalize_robot_orientation": True
    #             })
    # prop_space = OmegaConf.load('conf/datamodule/proprioception_dims/robot_no_joints.yaml')

    #import the transforms from conf/datamodule/default

    # transforms = OmegaConf.load('conf/datamodule/transforms/play_basic.yaml')
    # print(OmegaConf.to_yaml(transforms))

    # Initialize the PolicyGuideDataset
    print("Loading dataset...")
    # print(f"Config: {OmegaConf.to_yaml(cfg)}")
    dm = PolicyGuideDataModule(datasets=cfg.datasets, training_repo_root="/home/choudhue/PolicyGuide/dataset/calvin_D_3T_dataset", root_data_dir="/home/choudhue/PolicyGuide/dataset/calvin_D_3T_dataset", transforms=cfg.transforms)
    # print(cfg.datasets)
    dm.setup()  # or just `setup()` if you're not specifying stage

    dataset = dm.train_dataloader()

    # dataset = PolicyGuideDataset(dataset_dir=zarr_path, obs_space=obs_space, proprio_state=prop_space, abs_datasets_dir=zarr_path, lang_folder="lang_annotations", transforms=transforms)
    ...
    for idx, batch in enumerate(dataset):
        ...
        for i in range(batch["observation.image_static"].shape[0]):
            ...
            img = img.permute(1, 2, 0).numpy().astype(np.uint8) # Convert from (C, H, W) to (H, W, C)
            img = img * 255
            print(img)
